 ...
 *
 */
template< class Key, class Data, class Sizefn = Countfn< Data > , class Deletefn = NullDeletefn< Key,Data > > class LRUCache {
	public:
	...
	private:
		...
		inline void _remove( const Map_Iter &miter ) {
			...
			Deletefn()(miter->second->first, miter->second->second);
